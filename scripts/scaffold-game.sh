...
echo "  1. Spiellogik in $GAME_ID/index.html anpassen"
echo "     (das Demo-Spiel 'Tippe auf die richtige Farbe' ersetzen)."
echo "     running-Flag + timer-Handle + stopGame() sind schon eingebaut — behalten!"
echo "  2. Eigenes Icon zeichnen (icon.svg, icon-maskable.svg)"
echo "  3. Eintrag in games.json ergaenzen (Kategorie, Titel, Beschreibung, Alter),"
echo "     dann:  python scripts/sync-games.py"
echo "     -> setzt Landing-Karte, bumpt Sammlung-SW-Cache und Spielzahl automatisch."
echo "  4. Pruefen:"
echo "       python scripts/check-spiele.py            # Regeln"
echo "       python scripts/check-syntax.py            # JS-Syntax"
echo "       node   scripts/smoke-test.mjs $GAME_ID    # Headless-Durchlauf"
echo ""
